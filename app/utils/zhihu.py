...
def user_activities(user_id="lu-xin-qiong"):
    ...
    while not is_end:
        for activity in activities:
            ...
            if activity['action_text'] == "回答了问题" or activity['action_text'] == "赞同了回答" or activity[
                'action_text'] == '收藏了回答':
                print(activity['action_text'], activity['target']['question']['title'])
            elif activity['action_text'] == "赞同了文章" or activity['action_text'] == "关注了问题" or activity[
                'action_text'] == '收藏了文章' or activity['action_text'] == '发表了文章':
...
def hot_list_format():
    ...
    for hot in hot_list:
        ...
        print(info['id'], info["question"]['answer_count'], info['author']['name'],
              info['author']['url_token'] if info['author']['name'] != '匿名用户' else '')


def get_all_following(user_id='zhiyiYo'):
    data = api.follower_info(user_id=user_id)
    followings = data['data']
    page = data['paging']
    is_end = False
    all_follwings = []
    all_follwings.extend(followings)
    while not is_end:
        next_url = page['next']
        next_offset = next_url.split('=')[2]
        is_end = page['is_end']
        data = api.following_info(user_id=user_id, offset=next_offset)
        # print(data)
        if data['paging']['next'] == next_url:
            break
        followers = data['data']
        all_follwings.extend(followers)
        page = data['paging']

    return all_follwings


def get_all_follower(user_id='zhiyiYo'):
    data = api.follower_info(user_id=user_id)
    followers = data['data']
    page = data['paging']
    is_end = False
...
def user_all_answers(user_id):
    data = api.get_user_answer(user_id)
    answers = data['data']
    page = data['paging']
    is_end = False
    all_answers = []
    ...
    try:
        while not is_end:
            ...
            next_offset = next_url.split('=')[2]
            is_end = page['is_end']
            data = api.get_user_answer(user_id=user_id, offset=next_offset)
            if data['paging']['next'] == next_url:
                break
            answers = data['data']
            all_answers.extend(answers)
...
def topic_all_discussions(topic_id):
    data = api.topic_get(topic_id=topic_id)
    ...
    try:
        while not is_end:
            ...
            next_offset = next_url.split('=')[1].split('&')[0]
            is_end = page['is_end']
            data = api.topic_get(topic_id=topic_id, offset=next_offset)
            if data['paging']['next'] == next_url:
                break
            discussions = data['data']
            all_discussions.extend(discussions)
...
def question_all_answers(question_id):
    data = api.get_question_answer(question_id=question_id)
    answers = data['data']
    page = data['paging']
    is_end = False
    all_answers = []
    ...
    while not is_end:
        ...
        is_end = page['is_end']
        data = api.get_question_answer(question_id=question_id, offset=next_offset)
        if data['paging']['next'] == next_url:
            break
        answers = data['data']
        all_answers.extend(answers)
